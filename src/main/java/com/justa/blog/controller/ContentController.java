...
import com.justa.common.config.Constant;
import com.justa.common.controller.BaseController;
...
public class ContentController extends BaseController {
	...
	String edit(@PathVariable("cid") Long cid, Model model) {
		...
		String wecharId = bContentDO.getWechatid();
		List<WechatOfficialAccountsDO> list = wechatOfficialAccountsService.list(null);
		for(WechatOfficialAccountsDO w : list) {
			if(w.getWechatid().equals(wecharId)) {
				w.setRemarks("selected");
				break;
			}
		}
		model.addAttribute("bContent", bContentDO);
		model.addAttribute("wechatids", list);
	...
	public R save(ContentDO bContent) {
		...
		WechatOfficialAccountsDO wechat = wechatOfficialAccountsService.getByWechatId(bContent.getWechatid());
		bContent.setAuthor(wechat.getName());
		int count;
